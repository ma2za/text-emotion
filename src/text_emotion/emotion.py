...
CONFIG = {"model": {"name": "ma2za/roberta-emotion", "lang": ["en"]}}
...
def is_translate_available():
    return (
        ...
        and importlib.util.find_spec("easynmt") is not None
    )
...
class Detector:
    def __init__(self, emotion_language: str = "en", batch_size: int = 4):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            CONFIG.get("model", {}).get("name"), trust_remote_code=True
        )
        ...
        self.model = AutoModel.from_pretrained(
            CONFIG.get("model", {}).get("name"), trust_remote_code=True, config=config
        )
    ...
    def __language_detection(text: List[str]) -> List[str]:
        ...
        try:
            ...
            raise TextEmotionException(
                "The fasttext language detection model is not present!"
            )
        text = [t.replace("\n", " ") for t in text]
                    ...
                    list(
                        zip(
                            ...
                            self.translator.translate(
                                input_sentences, source_lang=src_lang, target_lang="en"
                            ),
                        )
                    )
                )
    ...
    def detect(self, text: Union[str, List[str]]) -> Union[str, List[str]]:
        ...
        with torch.no_grad():
            for i in range(int(len(inputs) / self.batch_size) + 1):
                ...
                prediction = (
                    self.model(input_ids).logits.argmax(-1).cpu().detach().numpy()
                )
        ...
        labels = self.translator.translate(
            output, source_lang="en", target_lang=self.emotion_language
        )
